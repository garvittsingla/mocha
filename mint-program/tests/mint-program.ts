...
import { MintTokenMplx } from "../target/types/mint_token_mplx";
import { PublicKey, Keypair, SystemProgram } from "@solana/web3.js";
import { assert } from "chai";
import { MPL_CORE_PROGRAM_ID } from "@metaplex-foundation/mpl-core";

//through describe , we describe a test suite.
describe("mint-program", () => {
  // Configure the client to use the local cluster.
  const provider = anchor.AnchorProvider.env();
  anchor.setProvider(provider);
  const program = anchor.workspace.MintTokenMplx as Program<MintTokenMplx>;
  ...
  //These things runs before any tests starts. that is to init things.
  //like air dropping sols.
  before(async () => {
    // Fund accounts and confirm each airdrop before proceeding
    const confirmAirdrop = async (publicKey: PublicKey) => {
      const sig = await provider.connection.requestAirdrop(publicKey, 2_000_000_000);
      const { blockhash, lastValidBlockHeight } = await connection.getLatestBlockhash();
      await connection.confirmTransaction({ signature: sig, blockhash, lastValidBlockHeight }, "confirmed");
    };
    await confirmAirdrop(creator.publicKey);
    await confirmAirdrop(nonWhitelistedCreator.publicKey);
    await confirmAirdrop(unauthorizedAuthority.publicKey);
    ...
    // Derive ProgramData address by reading the on-chain program account directly.
    // This avoids hardcoding the BPF Loader Upgradeable Program ID.
    const programAccountInfo = await connection.getParsedAccountInfo(program.programId);
  ...
  describe("WhitelistCreator", () => {
    it("Whitelist a creator", async () => {
      try {
        ...
        //if error logs are array log it iteratively.
    ...
    it("Fails to freeze with unauthorized authority", async () => {
      try {
        await program.methods
          .freezeNft()
          .accountsStrict({
            payer: unauthorizedAuthority.publicKey,
            asset: asset.publicKey,
            collection: collection.publicKey,
            collectionAuthority: collectionAuthorityPda,
            coreProgram: MPL_CORE_PROGRAM_ID,
